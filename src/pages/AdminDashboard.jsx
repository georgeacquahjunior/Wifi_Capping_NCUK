...
export default function AdminDashboard() {
  ...
  // Fetch from backend instead of JSON
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const res = await fetch("https://wifi-capping-ncuk.onrender.com/usage_logs/dashboard");
  ...
  return (
    <div className="dashboard-container">
      ...
      {exceededStudents.length > 0 && (
        <div className="alert">
          {exceededStudents.length} student
          {exceededStudents.length > 1 ? "s have" : " has"} exceeded the data cap!
        </div>
      )}
      ...
      <table>
        ...
        <tbody>
          {filteredStudents.map((student) => {
            ...
            const dataLeft = dataCap - student.usage;
            ...
            return (
              ...
              >
                ...
                <td>{student.dateAllocated || "—"}</td>
                <td>{student.usage} GB</td>
                <td>{dataLeft >= 0 ? dataLeft : 0} GB</td>
                <td>{percent}%</td>
                <td>
                  <span className={status.toLowerCase()}>{status}</span>
                </td>
